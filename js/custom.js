    ...
    contactForm.submit(function(e) {
        ...
        contactForm.find('.requiredField').each(function() {
            if ($.trim($(this).val()) == '') {
                ...
                alertWrapper.append('<div class="alert">Dato obbligatorio: ' + labelText + '.</div>');
                hasError = true;
            } else if ($(this).hasClass('email')) {
                var emailReg = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
                if (!emailReg.test($.trim($(this).val()))) {
                    var labelText = $(this).attr('placeholder');
                    alertWrapper.append('<div class="alert"> Informazione non valida ' + labelText + '.</div>');
                    hasError = true;
                }
            }
        });
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status == "error") {
                        ajaxError = true;
                        contactForm.append('<div class="alert"><strong>Errore</strong> Si &egrave; verificato un problema durante l\'invio del messaggio!</div>');
                    } else if (data.status == 'ok') {
                        contactForm.slideUp(300, function() {
                            $(this).before('<div class="alert"><strong>Grazie</strong> La vostra richiesta &egrave; stata inviata correttamente. </div>');
                        });
                    }
                },
                error: function() {
                    //Ajax request success
                    ajaxError = true;
                    $('form#contact-form').append('<div class="alert"><strong>Errore</strong> Si &egrave; verificato un problema durante l\'invio del messaggio!</div>');
                }
            });
        }
        if (ajaxError) {
